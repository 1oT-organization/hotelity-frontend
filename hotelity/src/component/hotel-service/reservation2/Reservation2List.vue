...
function updateSelectedMonth(newMonth) {
  ...
  console.log("updateSelectedMonth 실행됨");
...
function handleDateClicked(formattedDate) {
  ...
  if (obj[formattedDate]) {
    ...
    reservations.value = obj[formattedDate]; // Update reservations with the reservations of the selected date
  } else {
    console.log("No reservations for " + formattedDate);
    reservations.value = []; // Clear reservations if there are no reservations for the selected date
  }
}
